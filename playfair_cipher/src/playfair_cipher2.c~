...
int main(int argc, char* argv[]) {
  ...
  char* cipherText, *fileName;

	fileName = "input.txt";
  strcpy(cipherText, leEntrada(fileName));
 	printf("String de entrada: %s \n",cipherText);
  
  stringToLowerCase(cipherText, strlen(cipherText)-1);
...
void digraphInitialize() {
  ...
  inputFile = fopen("digraphsInput.txt", "r");
  float value;
...
void trigraphInitialize() {
  ...
  inputFile = fopen("trigraphsInput.txt", "r");
