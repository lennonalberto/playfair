...
int main(int argc, char**argv) {
  ...
  if (argc > 3) {
    ...
    stringEncriptada = calloc(2*strlen(stringOriginal), sizeof(char));
    num = limpaTexto(stringOriginal, stringLimpa);
    if(modo[0] == '0'){
    	encriptar(stringLimpa, stringEncriptada, num);
    	printf("Texto Claro  : ");
   		puts(stringLimpa);
   		printf("Texto Cifrado: ");
   		puts(stringEncriptada);
    	
    }
    else {
    	decriptar(stringLimpa, stringEncriptada, num);
     	printf("Texto Cifrado  : ");
   		puts(stringLimpa);
   		printf("Texto Decifrado: ");
   		puts(stringEncriptada);
   	}
   	salvaArquivo(stringEncriptada);
	}
  else {
  modoUso();
  return 1;
